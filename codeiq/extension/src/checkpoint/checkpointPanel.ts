...
import { generateOverview } from "./questionGenerator";
...
export class CheckpointPanel {
  ...
  private render(block: CodeBlock, question: string): string {
    ...
    return `<!doctype html><html><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}';"><style>${styles}</style></head><body>
      ...
      <section class="context"><div class="context-top"><strong>${escapeHtml(block.filePath)}</strong><span>Lines ${block.startLine}–${block.endLine} · Risk ${block.riskScore}/100</span></div><div class="provenance">${block.source === "ai_agent_detected" ? "Likely AI-assisted or pasted" : "Likely pasted"} <em>· heuristic, not confirmed</em></div><pre>${excerpt}</pre><div class="reasons">${block.riskReasons.map((reason) => `<span>${escapeHtml(reason)}</span>`).join("")}</div></section>
      <section class="explanation"><div class="eyebrow">AUTOMATIC EXPLANATION</div><p>${escapeHtml(generateOverview(block.excerpt || "", block.filePath.split(/[\\\\/]/).pop() || "this block"))}</p></section><section class="question"><div class="eyebrow">NOW, EXPLAIN IT IN YOUR OWN WORDS</div><h2 id="question">${escapeHtml(question)}</h2><textarea id="explanation" placeholder="Explain the behavior in your own words…" rows="5"></textarea><div class="actions"><button class="primary" id="verify">Verify understanding</button>${voice ? '<button class="secondary" id="voice">Voice input</button>' : ""}</div><p class="privacy">Local-first by default. Code is only sent to an evaluator when you explicitly opt in through settings.</p></section>
