...
import { PremiumCheckoutModal, type CheckoutProduct } from "@/components/PremiumCheckoutModal";
...
function ProductDetailsPage() {
    ...
    return (
      <div className="min-h-screen bg-background pt-32 pb-24 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-muted-foreground mb-8">
          The luxury item you are looking for does not exist or is no longer available.
        </p>
        <Link
          to="/products"
          className="bg-gold text-primary-foreground px-6 py-3 rounded-full font-medium shadow-[0_0_20px_rgba(212,160,23,0.3)] hover:shadow-[0_0_30px_rgba(212,160,23,0.5)] transition-all"
        >
          Return to Catalog
        </Link>
      </div>
  ...
  // Build checkout product payload
            ...
            {product.images.length > 1 && (
              <div className="grid grid-cols-4 gap-4">
                {product.images.slice(1, 5).map((img, idx) => (
                  <div
                    key={idx}
                    className="aspect-square rounded-xl overflow-hidden border border-border/40 cursor-pointer hover:border-gold/50 transition-colors"
                  >
              ...
              <button
                ...
                className={`flex-1 px-8 py-4 rounded-2xl font-semibold flex items-center justify-center gap-2 transition-all ${
                  product.stock_status === "Out of Stock"
                    ...
                    : "bg-gold text-primary-foreground hover:bg-gold/90 shadow-[0_0_20px_rgba(212,160,23,0.3)] hover:shadow-[0_0_30px_rgba(212,160,23,0.5)] active:scale-[0.98]"
                }`}
              >
              ...
              <div className="flex items-start gap-4 p-4 rounded-2xl bg-card/20 backdrop-blur-sm border border-border/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Precision cut from the finest aerospace-grade materials.
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-4 p-4 rounded-2xl bg-card/20 backdrop-blur-sm border border-border/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Fast, secure shipping across all major pin codes.
                  </p>
                </div>
              </div>
            </div>
